...
func NewObjectOptionsModel(mode, columns, order string) *ObjectOptionsModel {
    ...
    if mode == "fit" { m.modeIdx = 1 }
    if columns == "wide" { m.columnsIdx = 1 }
    switch order {
    case "name": m.orderIdx = 0
    case "-name": m.orderIdx = 1
    case "creation": m.orderIdx = 2
    case "-creation": m.orderIdx = 3
    default: m.orderIdx = 0
    }
    return m
}
...
func (m *ObjectOptionsModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.KeyMsg:
        switch t.String() {
        case "up", "k": if m.focus > 0 { m.focus-- }
        case "down", "j": if m.focus < 2 { m.focus++ }
        case "left", "right", " ", "space":
            ...
            case 2: // order
                if t.String() == "left" {
                    if m.orderIdx > 0 { m.orderIdx-- } else { m.orderIdx = len(objOrderKeys)-1 }
                } else {
                    if m.orderIdx < len(objOrderKeys)-1 { m.orderIdx++ } else { m.orderIdx = 0 }
                }
            }
            return m, nil
        case "ctrl+s":
            return m, func() tea.Msg { return ObjectOptionsChangedMsg{TableMode: objModeKeys[m.modeIdx], Columns: objColumnsKeys[m.columnsIdx], ObjectsOrder: objOrderKeys[m.orderIdx], SaveDefault: true} }
        case "enter":
            return m, func() tea.Msg { return ObjectOptionsChangedMsg{TableMode: objModeKeys[m.modeIdx], Columns: objColumnsKeys[m.columnsIdx], ObjectsOrder: objOrderKeys[m.orderIdx], Accept: true, Close: true} }
        }
    }
    return m, nil
}
