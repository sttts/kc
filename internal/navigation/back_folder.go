...
func (b *backFolder) ObjectListMeta() (schema.GroupVersionResource, string, bool) {
    ...
    if mp, ok := b.inner.(metaProv); ok { return mp.ObjectListMeta() }
...
func (b *backFolder) Len() int {
    if !b.hasBack { return b.inner.Len() }
    return b.inner.Len() + 1
}

func (b *backFolder) Lines(top, num int) []table.Row {
    if num <= 0 { return nil }
    if !b.hasBack { return b.inner.Lines(top, num) }
    // With back: index 0 is BackItem, following are inner rows shifted by 1.
    if top <= 0 {
        // include back then inner starting at 0
        out := make([]table.Row, 0, num)
        out = append(out, BackItem{})
        if num-1 > 0 { out = append(out, b.inner.Lines(0, num-1)...)}
        return out
    }
    // top > 0 -> map to inner starting at top-1
    return b.inner.Lines(top-1, num)
}

func (b *backFolder) Above(rowID string, num int) []table.Row {
    if !b.hasBack || num <= 0 { return b.inner.Above(rowID, num) }
    if rowID == "__back__" { return nil }
    return b.inner.Above(rowID, num)
}

func (b *backFolder) Below(rowID string, num int) []table.Row {
    if !b.hasBack || num <= 0 { return b.inner.Below(rowID, num) }
    if rowID == "__back__" { return b.inner.Lines(0, num) }
    return b.inner.Below(rowID, num)
}

func (b *backFolder) Find(rowID string) (int, table.Row, bool) {
    if !b.hasBack { return b.inner.Find(rowID) }
    if rowID == "__back__" { return 0, BackItem{}, true }
