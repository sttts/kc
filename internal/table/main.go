...
- Every cell has a style (even when "neutral").
- All base strings are ASCII.
- Truncate plain text first, then apply style (ANSI), to avoid slicing escape codes.
*/
...
func (m *BigTable) refreshRowsOnly() {
    m.rebuildWindow()
}

// rebuildWindow sets the table rows to the current window [top:top+height)
// and positions the table cursor at (cursor-top).
...
func (m *BigTable) applyMode() {
    ...
    case ModeScroll:
        ...
        // Style every cell AFTER sizing; no truncation here.
        m.rebuildWindow()
...
func measurePlainWidthsFromProvider(cols []table.Column, list List) []int {
    n := len(cols)
    w := make([]int, n)
...
func (m *BigTable) repositionOnDataChange() {
    ...
    if m.focusedID == "" {
        // Nothing tracked yet: clamp current cursor.
        if m.cursor >= n { m.cursor = n-1 }
        if m.cursor < 0 { m.cursor = 0 }
        return
    }
    if idx, _, ok := m.list.Find(m.focusedID); ok {
        // Still present: keep index.
        m.cursor = idx
        return
    }
    // Try the next row after the vanished one.
    ...
    // Otherwise try the previous row.
