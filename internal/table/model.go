...
type List interface {
	Lines(ctx context.Context, top, num int) []Row
	Above(ctx context.Context, rowID string, num int) []Row
	Below(ctx context.Context, rowID string, num int) []Row
	Len(ctx context.Context) int
	// Find returns the absolute index and row for a given ID if present.
	Find(ctx context.Context, rowID string) (int, Row, bool)
}
...
func (l *SliceList) Prepend(newRows ...Row) {
	l.rows = append(append([]Row(nil), newRows...), l.rows...)
	l.rebuildIndex()
}
...
func (l *SliceList) Len(context.Context) int { return len(l.rows) }

func (l *SliceList) Lines(_ context.Context, top, num int) []Row {
	if num <= 0 || top >= len(l.rows) {
		return nil
	}
	if top < 0 {
		top = 0
...
func (l *SliceList) Above(_ context.Context, rowID string, num int) []Row {
	i, ok := l.index[rowID]
	if !ok {
		return nil
	}
	...
	for j := start; j < i; j++ {
		out[j-start] = l.rows[j]
	}
	return out
}

func (l *SliceList) Below(_ context.Context, rowID string, num int) []Row {
	i, ok := l.index[rowID]
	if !ok {
		return nil
	}
...
func (l *SliceList) Find(_ context.Context, rowID string) (int, Row, bool) {
	i, ok := l.index[rowID]
	if !ok || i < 0 || i >= len(l.rows) {
		return -1, nil, false
	}
