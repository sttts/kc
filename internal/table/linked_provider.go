...
func (l *LinkedList) Append(rows ...Row) {
	for _, r := range rows {
		l.appendOne(r)
	}
}
...
func (l *LinkedList) Len(context.Context) int { return l.size }

// Lines returns a slice of rows starting at top with length up to num.
func (l *LinkedList) Lines(_ context.Context, top, num int) []Row {
	if num <= 0 || top >= l.size {
		return nil
	}
	if top < 0 {
...
func (l *LinkedList) Above(_ context.Context, rowID string, num int) []Row {
	n, ok := l.byID[rowID]
	if !ok || num <= 0 {
		return nil
	}
...
func (l *LinkedList) Below(_ context.Context, rowID string, num int) []Row {
	n, ok := l.byID[rowID]
	if !ok || num <= 0 {
		return nil
	}
...
func (l *LinkedList) Find(_ context.Context, rowID string) (int, Row, bool) {
	n, ok := l.byID[rowID]
	if !ok {
		return -1, nil, false
	}
