...
func (g Grid) makeMessage(wDims, hDims []int) BubbleLayoutMsg {
	...
	xOffsets := make([]int, len(wDims))
	for i := 1; i < len(wDims); i++ {
		xOffsets[i] = xOffsets[i-1] + wDims[i-1]
	}
	yOffsets := make([]int, len(hDims))
	for i := 1; i < len(hDims); i++ {
		yOffsets[i] = yOffsets[i-1] + hDims[i-1]
	}
	...
	for rowIdx, row := range g {
		for colIdx, l := range row {
			...
			if _, ok := idRow[l.id]; !ok {
				idRow[l.id] = rowIdx
				if rowIdx < len(yOffsets) {
					msg.size[l.id].Y = yOffsets[rowIdx]
				}
			}
			if _, ok := idCol[l.id]; !ok {
				idCol[l.id] = colIdx
				if colIdx < len(xOffsets) {
					msg.size[l.id].X = xOffsets[colIdx]
				}
			}
