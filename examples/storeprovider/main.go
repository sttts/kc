...
func main() {
	...
	// Create a cluster pool with 2m TTL and 30s discovery refresh, then start it
	pool := kccluster.NewPool(2*time.Minute, 30*time.Second)
	pool.Start()
	defer pool.Stop()
