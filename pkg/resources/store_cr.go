...
func (s *CRReadOnlyStore) List(ctx context.Context, key StoreKey) (*unstructured.UnstructuredList, error) {
	...
	if key.Namespace != "" {
		ri = dc.Resource(key.GVR).Namespace(key.Namespace)
	} else {
		ri = dc.Resource(key.GVR)
	}
...
func (s *CRReadOnlyStore) Watch(ctx context.Context, key StoreKey) (<-chan Event, context.CancelFunc, error) {
	...
	if err != nil {
		cancel()
		return nil, nil, fmt.Errorf("add handler: %w", err)
	}
...
type CRStoreProvider struct {
	pool *ClusterPool
	key  ClusterKey
}

func NewCRStoreProvider(pool *ClusterPool, key ClusterKey) *CRStoreProvider {
	return &CRStoreProvider{pool: pool, key: key}
}
