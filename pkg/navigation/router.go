...
func (r *SimpleRouter) Parse(raw string) (Path, error) {
    ...
    segs := splitClean(raw)
    p := Path{Raw: raw}
    if len(segs) == 0 { return p, nil }

    switch "/" + segs[0] {
    case RootCluster:
        // /cluster[/namespaces/<ns>/<resource>[/<name>]] or /cluster/<resource>[/<name>]
        if len(segs) >= 2 && segs[1] == "namespaces" {
            if len(segs) >= 3 { p.Namespace = segs[2] }
            if len(segs) >= 4 { p.Resource = segs[3] }
            if len(segs) >= 5 { p.Name = segs[4] }
        } else if len(segs) >= 2 {
            p.Resource = segs[1]
            if len(segs) >= 3 { p.Name = segs[2] }
        }
        return p, nil
    case RootContexts:
    ...
    case RootKubeconfigs:
        ...
        if len(segs) >= 2 { p.Kubeconfig = segs[1] }
        if len(segs) >= 4 && segs[2] == "contexts" { p.Context = segs[3] }
    ...
    case RootGroups:
        ...
        if len(segs) >= 2 { p.Group = segs[1] }
        if len(segs) >= 3 { p.Version = segs[2] }
        i := 3
        if len(segs) >= 5 && segs[3] == "namespaces" {
            p.Namespace = segs[4]
            i = 5
        }
        if len(segs) > i { p.Resource = segs[i] }
        if len(segs) > i+1 { p.Name = segs[i+1] }
        return p, nil
    default:
...
func (r *SimpleRouter) Build(p Path) string {
    ...
    if p.Group != "" || p.Version != "" {
        ...
        if p.Namespace != "" { parts = append(parts, "namespaces", p.Namespace) }
        if p.Resource != "" { parts = append(parts, p.Resource) }
        if p.Name != "" { parts = append(parts, p.Name) }
...
func (r *SimpleRouter) Parent(p Path) Path {
    ...
    if p.Resource != "" { p.Resource = ""; return p }
    if p.Namespace != "" { p.Namespace = ""; return p }
    if p.Version != "" { p.Version = ""; return p }
    if p.Group != "" { p.Group = ""; return p }
    // Collapse to roots
    p.Context = ""
    p.Kubeconfig = ""
    p.Raw = "/"
